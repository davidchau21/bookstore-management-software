 ...
 * @author DAVID
 */
public class GUI_ThongKeSanPham extends javax.swing.JPanel {
     ...
     */
    public GUI_ThongKeSanPham() throws Exception {
        initComponents();
...
//			renderLoadData();
    ...
    public void renderLoadData() {
		ArrayList<SanPham> list = dao_SanPham.thongKeSPBanChay(30);
		...
		for (SanPham sp : list) {
			...
			modelTK.addRow(new Object[] {
					sp.getMaSanPham(),sp.getTenSanPham(), loaisp,sp.getGiaNhap(),sp.getSoLuong(),sp.getNhaXB().getMaNXB(),tt
			});
    ...
    public void renderData(Date tuNgay, Date toiNgay) {
		...
		if(soNgay <= 3) {
			...
			minSoLuong = 200;
		ArrayList<SanPham> list = dao_SanPham.thongKeSPBanChay(tuNgay, toiNgay, minSoLuong);
		modelTK.setRowCount(0);
		tableBanChay.clearSelection();
		modelTK.getDataVector().removeAllElements();
		soLuongDaBan = 0;
		...
		for (SanPham sp : list) {
			...
			if(sp.getSoLuong() < 10)
				...
				tt = "";
			modelTK.addRow(new Object[] {
					sp.getMaSanPham(),sp.getTenSanPham(), loaisp,df.format(sp.getGiaNhap()),sp.getSoLuong(),sp.getNhaXB().getMaNXB(),tt
			});
			// lay cot nxb bien thanh cot soLuongDaBan ben sql da xu ly roi
			soLuongSP += Integer.parseInt(sp.getNhaXB().getMaNXB());
		}
    ...
    private void initComponents() {
        ...
        tableBanChay.setModel(modelTK = new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã SP", "Tên SP", "Tên loại", "Đơn giá", "Số lượng tồn", "Số lượng đã bán","Trạng thái"
            }
        ));
