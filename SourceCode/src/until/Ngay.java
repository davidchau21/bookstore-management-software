...
public class Ngay {
		...
		if (now.getDate() == 1) {
			if (now.getMonth() == 0)
				...
				return new Date(now.getYear(), now.getMonth() - 1,
						Ngay.getSoNgayTrongThang(now.getMonth(), now.getYear() + 1900));
		} else {
			return new Date(now.getYear(), now.getMonth(), now.getDay() - 1);
		}
	}
	...
	public static Pair<Date, Date> getRangeYear(int nam) {
		Date tuNgay = new Date(nam -1900, 0, 1);
		Date toiNgay = new Date(nam -1900, 11, 31);
